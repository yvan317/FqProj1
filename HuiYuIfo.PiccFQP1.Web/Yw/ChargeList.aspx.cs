...
{
    public partial class ChargeList : BaseListWebFX<YwCyh, YwCyhController>
    {
        ...
        public override void AfterSubmit()
        {
            switch (Task.Command)
            {
                ...
                case "exportExample":
                    ...
                    //Response.ContentType = "application/vnd.ms-excel";
                    //设置下载的Excel文件名
                    Response.AddHeader("Content-Disposition", string.Format("attachment;filename={0}", "cyhExample.xlsx"));
                    //Clear方法删除所有缓存中的HTML输出。但此方法只删除Response显示输入信息，不删除Response头信息。以免影响导出数据的完整性。
                    Response.Clear();
